...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        local_neigh_size = 1 # NHI: default local neighbor distance = 1
    ):
        ...
        # Decoder Hidden
        if pooling_type:
            input_dim = encoder_h_dim + bottleneck_dim + 45 #(2*local_neigh_size+1)**2  #NHI: when pooling also add local info  NOW GRAPH WARNING HARD CODE
        else:
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, all_local_info, user_noise=None):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
